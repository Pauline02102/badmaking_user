...
async function sendResetEmail(userEmail, resetUrl) {
  const transporter = nodemailer.createTransport({
    service: 'hotmail', // ou un autre service de messagerie
    auth: {
      user: 'vdhpauline@hotmail.com', //  adresse email
      pass: process.env.MDP_MAIL //  mot de passe email ou jeton d'authentification
    },
  });

  const mailOptions = {
    from: 'vdhpauline@hotmail.com',
    to: userEmail,
...
router.post('/request-reset-password', async (req, res) => {
  try {
    ...
    // Définir une date d'expiration pour le jeton
    const expireTime = Date.now() + 3600000; // 1 heure

    // Enregistrer le jeton et la date d'expiration dans la base de données pour cet utilisateur
    user.resetPasswordToken = resetToken;
    user.resetPasswordExpires = expireTime;
    await user.save();

    // Envoyer le jeton par email à l'utilisateur
    const resetUrl = `https://rixbad.ovh/reset-password/${resetToken}`;
    // Fonction d'envoi d'email (à implémenter)
    await sendResetEmail(user.email, resetUrl);

    res.send('Un email de réinitialisation de mot de passe a été envoyé.');
  } catch (error) {
